...
def afficher_description():
    ...
    if option == "Iris":
        ...
        for i, heading in enumerate(column_headings):
            ...
            donnees_treeview.column(i, width=140)  # Définir la largeur de la colonne
    elif option == "Maladie":
        ...
        for i, heading in enumerate(column_headings):
            donnees_treeview.heading(i, text=heading)
            donnees_treeview.column(i, width=90)  # Définir la largeur de la colonne
        ...
        maladie_data_subset = penguins_data.sample(10)
        for _, row in maladie_data_subset.iterrows():
            donnees_treeview.insert("", "end", values=row.tolist())
        # Définir les en-têtes des colonnes pour l'ensemble de données Maladie
            column_headings = penguins_data.columns.tolist()
        ...
        maladie_data_subset = Diabetes_data.sample(10)
        for _, row in maladie_data_subset.iterrows():
            donnees_treeview.insert("", "end", values=row.tolist())
        # Définir les en-têtes des colonnes pour l'ensemble de données diabets
            column_headings = Diabetes_data.columns.tolist()
        # Définir le titre de l'onglet 2 pour l'ensemble de données Diabetes
        notebook.tab(onglet2, text="Diabetes Dataset")
